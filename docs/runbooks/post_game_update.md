...
`web/public/data/Arsenals.csv` is the single source of truth for player identity. All post-game stats writes resolve names and aliases back to a canonical `playerId` before writing JSON.
...
- `web/public/stats/players-by-id/<playerId>/<season>/<gameId>.json`
- `web/public/stats/seasons/<season>/games.json`
- `web/public/data/<playerId>/<dateId>/outing_meta.json` (if `--outing-map` provided)

Player stat payloads store `playerId` explicitly. Season indexes store `playerIdsIncluded`. The legacy `web/public/stats/players/` slug tree and `players/index.json` are migration-only inputs and should not be recreated.
...
Everything under `web/public/stats/` is generated by these scripts. The files are committed to git so Vercel can serve them. To regenerate, re-run `scripts/post_game_update.py` (or `scripts/import_boxscore.py` for a single game). Use the `post-game-stats-importer` skill to import, validate, commit, and push in one step.
