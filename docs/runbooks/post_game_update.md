...
Everything under `web/public/stats/` is generated by these scripts. The files are committed to git so Vercel can serve them. To regenerate, re-run `scripts/post_game_update.py` (or `scripts/import_boxscore.py` for a single game). Use the `post-game-stats-importer` skill to import, commit, and push in one step.
