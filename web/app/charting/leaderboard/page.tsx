  ...
  try {
    ...
    if (tab === "pitchers") {
      ...
      for (const segment of segments) {
        if (segment.playerId && segment.playerId.trim() !== "") {
          pitcherMap.set(segment.playerId, segment.displayName);
        }
      }
      ...
      const uniqueHitters = Array.from(
        new Set(
          pas
            .map((plateAppearance) => plateAppearance.hitterName.trim())
            .filter((name) => name.length > 0),
        ),
      );

      const hitterPromises: Array<Promise<HitterLeaderboardRow | null>> = uniqueHitters.map(
        async (name) => {
