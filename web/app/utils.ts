...
/** Unique sorted pitch types from data */
export function uniqueTypes(pitches: Pitch[]): string[] {
  return [...new Set(pitches.map((p) => p.pitch_type))].sort();
}

/** Unique sorted quadrants from data */
export function uniqueQuadrants(pitches: Pitch[]): string[] {
  return [...new Set(pitches.map((p) => p.result_quadrant))].sort();
}
