...
- Account magic links require `ACCOUNT_EMAIL_WEBHOOK_URL` in production. The webhook receives `{ to, subject, text }`; if `ACCOUNT_EMAIL_WEBHOOK_SECRET` is set, requests include `Authorization: Bearer <secret>`.
