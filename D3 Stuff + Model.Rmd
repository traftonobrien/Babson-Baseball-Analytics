---
title: "DHD Stuff+ Pipeline — XGBoost Shape Model"
author: "Diamond Hitting Development"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
## 4.2 — Within-Type Z-Scores (Splitter pooled with ChangeUp)
...
## 4.3 — Fastball Baseline Per Outing
...
### Step 2: Fix the Splitter / Offspeed Scoring (Section 5)
```{r}

```


## 4.4 — Differentials From Fastball
...
# 5 — Synthetic Target: ShapeImpactScore
...
# NEW: Global rank for absolute ceiling (Raw Arm Strength)
dt[, max_fb_velo_z     := robust_z(max_fb_velo)]

# 1. CATEGORIZE PITCHES PROPERLY
is_fb <- as.integer(grepl("fastball|sinker|four", dt$pitch_type, ignore.case = TRUE))
is_bb <- as.integer(grepl("slider|curveball|cutter", dt$pitch_type, ignore.case = TRUE))
is_os <- as.integer(grepl("changeup|splitter", dt$pitch_type, ignore.case = TRUE))

# 2. Fastball Target: Velo is king (Added 10% weight to Max Velo)
fb_score <- (
  0.35 * dt$fb_velo_pct_max_z +
  0.30 * dt$global_velo_z +
  0.10 * dt$max_fb_velo_z +      # NEW: 10% boost for raw arm strength
  0.15 * dt$movement_z +
  0.10 * dt$spin_z
)

# 3. Breaking Target: Big sweep, massive drop, high spin
bb_score <- (
  0.25 * dt$movement_z +
  0.20 * (-1 * dt$velo_diff_z) +   
  0.20 * dt$ivb_abs_z +            
...
# 4. Offspeed Target: Kill spin, kill IVB, massive velo drop
os_score <- (
...
# Combine all 3 buckets
dt[, ShapeImpactScore := is_fb * fb_score + is_bb * bb_score + is_os * os_score]
...
# 6 — XGBoost Model Training (Regularized + 5-Fold CV)

## 6.1 — Feature Selection (Correlation Pruning)
...
## 6.2 — 5-Fold Cross-Validation for Hyperparameter Selection
...
## 6.3 — Train Final Model + Holdout Evaluation
...
arsenal <- dt[, .(
  ...
  avg_velo_mph   = round(mean(avg_velo_mph, na.rm = TRUE), 1), # <-- Added Velo
  max_fb_velo    = round(max(max_fb_velo, na.rm = TRUE), 1),   # <-- Added Max Velo
  n_sessions     = uniqueN(date)
), by = .(player_id, pitch_type)]

# Sort by Player Name (A-Z), then by their best Stuff+ score
